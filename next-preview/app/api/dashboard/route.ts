...
export async function GET(): Promise<NextResponse> {
  ...
  try {
    const result = await getDocsForRequest(); // Azure AD → token delegated; Teams SSO → app-only read
    ...
    console.log(`[PERF] api-dashboard ${(performance.now() - t0).toFixed(0)}ms source=${result.source} docs=${docs.length}`);
